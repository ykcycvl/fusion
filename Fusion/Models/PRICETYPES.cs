//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Fusion.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class PRICETYPES
    {
        public int SIFR { get; set; }
        public string GUIDSTRING { get; set; }
        public Nullable<int> CODE { get; set; }
        public string NAME { get; set; }
        public string ALTNAME { get; set; }
        public Nullable<int> PARENTPRICETYPE { get; set; }
        public Nullable<short> ROUNDRULE { get; set; }
        public Nullable<int> FLAGS { get; set; }
        public Nullable<double> DEFCONSUMPERCENT { get; set; }
        public Nullable<double> DEFBONUSPERCENT { get; set; }
        public Nullable<short> STATUS { get; set; }
        public Nullable<short> EDITRIGHT { get; set; }
        public Nullable<double> DEFCOMBOPERCENT { get; set; }
        public Nullable<short> PRICETYPETYPE { get; set; }
        public string SHCOMMODITYGROUP { get; set; }
        public string SHSTOREHOUSE { get; set; }
        public Nullable<int> SHSETTINGSLINK { get; set; }
        public string RECSTAMP { get; set; }
    }
}
